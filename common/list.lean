...
def list.zip_pad (a' b') : list α → list β → list (α × β)
| [] [] := []
| [] (b::bs) := (a',b)::(list.zip_pad [] bs)
...
def mem_filter_of_pred_and_mem {P : α → Prop} [HD : decidable_pred P] : 
  ∀ (as : list α) (a : α), P a → a ∈ as → a ∈ list.filter P as 
| [] a HP HM := by cases HM 
| (a'::as) a HP HM := 
  begin
